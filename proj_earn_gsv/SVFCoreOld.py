...
class Panorama():
    ...
    def fromLocation(self, lat, lon):
        url = GSV_API_URL + "/metadata?location=" + \
            str(lat) + "," + str(lon) + "&key=" + MyConfig.apikey
        try:
...
class GSVCapture():
    ...
    def classify(self, infile, outfile):
        ...
        input_image = cv2.resize(
            input_image, (self.input_shape[3], self.input_shape[2]))
    ...
    def initialize(self, useCUDA):
        segnetModel = SVFHOME + \
            "SegNet-Tutorial-master/Example_Models/segnet_model_driving_webdemo.prototxt"
        segnetWeights = SVFHOME + \
            "SegNet-Tutorial-master/Example_Models/segnet_weights_driving_webdemo.caffemodel"
        ...
        if os.path.exists(segnetWeights) == False:
            ...
            f1 = open(
                SVFHOME + "SegNet-Tutorial-master/Example_Models/segnet_weights_driving_webdemo_1.caffemodel", 'rb')
            f.write(f1.read())
            f1.close()
            f2 = open(
                SVFHOME + "SegNet-Tutorial-master/Example_Models/segnet_weights_driving_webdemo_2.caffemodel", 'rb')
            f.write(f2.read())
            f2.close()
            f3 = open(
                SVFHOME + "SegNet-Tutorial-master/Example_Models/segnet_weights_driving_webdemo_3.caffemodel", 'rb')
            f.write(f3.read())
            f3.close()
            f.close()
    ...
    def batchGetByID(self, outdir):
        ...
        for line in lines:
            ...
            fresult.write(str(result[0]) + "," +
                          str(result[1]) + "," + str(result[2]) + "\n")
            fresult.close()
            sys.stdout.write(
                "progress: " + "{:.2%}".format(taskid/numtask) + "\n")
            sys.stdout.flush()
